...
function memoize(func) {
  const memoizedValue = [];
  return function memoizedLast() {
    if (memoizedValue.length === 0) {
      memoizedValue.push(func());
    }
    return memoizedValue[0];
  };
}
...
function logger(func, logFunc) {
  return function loggerFunc(args) {
    const qargs = JSON.stringify(args);

    logFunc(`${func.name}(${qargs}) starts`);
    const result = func(args);
    logFunc(`${func.name}(${qargs}) ends`);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let startFromInner = startFrom;
  return function showId() {
    const returnedValue = startFromInner;
    startFromInner += 1;
